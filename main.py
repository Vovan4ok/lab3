...
def model_immitation():
    for i in range(modeling_durability):
        ...
        if np.random.rand() <= bad_feel_possibility:
            total_pages_copy = total_pages_for_this_day
            total_pages_for_this_day = 0

        if total_pages_for_this_day < max_pages:
            translated_pages_for_this_day = total_pages_for_this_day
            if total_pages_for_this_day == 0:
                burnt_pages_per_every_day.append(total_pages_copy)
            else:
        ...
        print("Кількість документів, які надійшли на переклад: " + str(docs_number))
        if total_pages_for_this_day == 0:
            print("Загальна кількість сторінок: " + str(total_pages_copy))
        else:
...
def build_histogramma():
    ...
    plt.bar(list(range(0, modeling_durability)), burnt_pages_per_every_day, color="red", label="Згоріли", bottom=translated_pages_per_every_day)
    plt.xlabel("Ітерація")
    plt.ylabel("Сторінок")
